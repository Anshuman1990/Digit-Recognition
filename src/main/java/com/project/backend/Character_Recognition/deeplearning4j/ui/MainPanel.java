...
public class MainPanel extends JPanel implements NumberDrawnListener {
    ...
    @Override
    public void drawingFinished(final BufferedImage image) throws IOException {
        BufferedImage img =  ImageIO.read(new File("D:\\images\\test3\\94_109.jpg"));

        final Rectangle bounds = ImageUtils.findBoundsOfBlackShape(img);
        final Dimension newDim = getScaledMnistDigitDimensions(bounds);

        // Get only the digit out from the rest of the image
        final BufferedImage cropped = img.getSubimage(bounds.x,
                bounds.y,
                bounds.width,
                bounds.height);
